...
def main(args):
    ...
    # create model
    model = STE_t4(weights=args.load_weights, input_size=args.feature_dim,
                  num_classes=dataset_Test.num_classes, window_size=args.window_size, 
                  vocab_size = args.vocab_size,
                  framerate=args.framerate, pool=args.pool).to(args.device)#.cuda()
    logging.info(model)
    ...
    if not args.test_only:
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=args.patience, eps = 1e-7, factor = 0.1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--SoccerNet_path',   required=False, type=str,   default="../../data/path/to/SoccerNet/",     help='Path for SoccerNet' )
    ...
    parser.add_argument('--device', required=False, type=str, default='cuda', help='Device to use for training (e.g., "cuda" or "cpu")')

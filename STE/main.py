...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan  8 09:45:58 2022
...
def main(args):
    ...
    # create model
    model = STE2(weights=args.load_weights, input_size=args.feature_dim,
                  num_classes=dataset_Test.num_classes, window_size=args.window_size, 
                  vocab_size = args.vocab_size,
                  framerate=args.framerate, pool=args.pool)
    logging.info(model)
    ...
    if not args.test_only:
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', verbose=True, patience=args.patience, eps = 1e-4, factor = 0.5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_weights',   required=True, type=str,   default=None,     help='weights to load' )
    ...
    parser.add_argument('--evaluation_frequency', required=False, type=int,   default=3,     help='Number of chunks per epoch' )
    parser.add_argument('--framerate', required=False, type=int,   default=1,     help='Framerate of the input features' )
    parser.add_argument('--window_size', required=False, type=int,   default=8,     help='Size of the chunk (in seconds)' )
    ...
    parser.add_argument('--NMS_window',       required=False, type=int,   default=18, help='NMS window in second' )
    ...
    parser.add_argument('--LR',       required=False, type=float,   default=2e-03, help='Learning Rate' )
    parser.add_argument('--LRe',       required=False, type=float,   default=1e-06, help='Learning Rate end' )
    parser.add_argument('--patience', required=False, type=int,   default=20,     help='Patience before reducing LR (ReduceLROnPlateau)' )
